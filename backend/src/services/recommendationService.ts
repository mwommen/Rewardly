...
const PERK_STOPWORDS = [
  // generic/legal
  "terms of service", "privacy", "do not sell", "adchoices", "security center",
  "card agreements", "site map", "financial education",
  // nav/cta
  "opens new", "open new", "in the same window", "view details", "see details",
  "learn more", "apply now", "limited time", "offer ends",
  // marketing fluff
  "popular credit cards", "our most popular", "more cash back", "more points",
  // cross-promo
  "credit card product page", "offers and promotions",
  // too generic
  "category page"
];
...
const KEEP_KEYWORDS = [
  ...
  "travel insurance", "trip delay", "trip cancellation", "baggage",
  "purchase protection", "extended warranty", "return protection",
  "rental car", "auto collision", "primary rental", "secondary rental",
  "doordash", "uber credit", "lyft", "instacart", "grubhub",
  "hotel credit", "airline credit", "travel credit",
  "statement credit",
  "points transfer", "transfer partners",
  "after $", "after you spend", // signup heuristic
  "0% intro", "intro apr",
  "priority boarding", "free checked bag",
  "no annual fee",
];

const REWARD_NUMBER = /\b(\d+(\.\d+)?)\s?(%|x)\b/i; // e.g., 5%, 3x
const NUM_WITH_CATEGORY = /\b(\d+(\.\d+)?)\s?(%|x).{0,25}\b(dining|restaurants|grocer|gas|travel|transit|drug|online)\b/i;
...
function scoreLine(s: string): number {
  ...
  // Useful nouns
  if (/\b(protection|insurance|warranty|credit|transfer|lounge|entry|precheck|boarding|bag|rental)\b/i.test(s)) score += 1;
...
function isMostlyNavOrCTA(s: string): boolean {
  const low = s.toLowerCase();
  return (/\b(opens?|view|see|apply|learn|shop|explore)\b/.test(low) && !REWARD_NUMBER.test(s))
      || EXTRA_DISCARD_PATTERNS.some(rx => rx.test(s));
}

function cleanPerks(input: string[] | undefined, maxLen = 140, maxItems = 8): string[] {
...
const CAT_SYNONYMS: Record<string, string[]> = {
  restaurants: ["restaurants", "dining", "food", "coffee", "cafes", "eating out"],
...
function issuerDefaultsToPoints(issuer?: string): boolean {
  ...
  return s.includes("chase") || s.includes("american express") || s.includes("amex") || s.includes("citi");
}
...
function parseRateUnknown(
  ...
  const toPoints = (n: number) => toCashEquivalent(unitHint ?? "points", n, issuer ?? "other");
...
export async function recommendAllBenefits(opts: {
  ...
  const { merchant, amount = 0, mcc, includeRotating = true, minRate = 0 } = opts;
  ...
  const results = cards.map((c: any) => {
    ...
    if (Array.isArray(c.rewardsByCategory)) {
      for (const e of normalizeArray<RewardEntry>(c.rewardsByCategory)) {
        ...
        normalizeArray(e.keys).forEach((k) => seenMatches.add(String(k).toLowerCase()));
      }
    }
    ...
    if (includeRotating) {
      for (const q of normalizeArray<RotatingQuarter>(c.rewardsRotating)) {
        const active = q.start && q.end ? now >= new Date(q.start) && now <= new Date(q.end) : true;
        ...
        for (const e of normalizeArray<RewardEntry>(q.categories)) {
          ...
          if (rEff > bestRate) {
            ...
            notes = [active ? "rotating (active)" : "rotating (uncertain window)"];
          }
          normalizeArray(e.keys).forEach((k) => seenMatches.add(String(k).toLowerCase()));
          conf = Math.max(0, conf - confPenalty - (q.activationRequired ? 0.05 : 0));
        }
      }
    }
  ...
  const filtered = results.filter(
    (r) => r.effectiveRate > minRate || (r.perks && r.perks.length) || r.signupOffer
  );
...
export async function recommendBestCards(opts: {
  ...
  const { merchant, amount = 0, mcc, includeRotating = true } = opts;
